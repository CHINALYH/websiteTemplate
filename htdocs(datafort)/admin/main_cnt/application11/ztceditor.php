<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST' and isset($_POST['mend'])){
		...
		$sql = "update `advert_info` set linkname = '$linkname',linkurl = '$linkurl',linkpic = '$linkpic',ztc = '$ztc',zy = '$zy' where id = '$id'";

		$result = $mysqli->query($sql);
		if(!$result){
			$handle = $myfun->_mysql_string('<span class="blue">[ 编辑对联 ]</span> <span class="red">[ 失败 ]</span> '.$row['linkname']);
			//记录操作日志（失败）
			...
			$handle = $myfun->_mysql_string('<span class="blue">[ 编辑对联 ]</span> <span class="green">[ 成功 ]</span> '.$row['linkname']);
			//记录操作日志（成功）
			$insert = "insert into `handle_log_info` (uid,handle,addtime,ip) values ('$uid','$handle','$addtime','$ip')";
			$mysqli->query($insert);
		}
            	...
            	<table class="table_all">
                  ...
                  <tbody>
                   ...
                   <tr>
                      ...
                      <td>
                      <input type="radio" name="ztc" id="nhdn" value="1" <?php if($row['ztc']==1){?>checked="checked"<?php }?> /> <label for="nhdn">是</label>&nbsp;
                        <input type="radio" name="ztc" id="yhdn" value="0" <?php if($row['ztc']==0){?>checked="checked"<?php }?> /> <label for="yhdn">否</label> 
                    ...
                    <tr>
                      ...
                      <td>
                        <input type="radio" name="zy" id="yhdn" value="0" <?php if($row['zy']==0){?>checked="checked"<?php }?> /> <label for="yhdn">左</label> &nbsp;
                        <input type="radio" name="zy" id="nhdn" value="1" <?php if($row['zy']==1){?>checked="checked"<?php }?> /> <label for="nhdn">右</label>
                      </td>
                    </tr>
                    <tr>
