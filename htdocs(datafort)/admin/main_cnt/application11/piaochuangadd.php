<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST' and isset($_POST['sub'])){
		...
		$sql = "insert into `advert_info` (linkname,linkurl,linkpic,type_id,ishdn,adduser,addtime) values ('$linkname','$linkurl','$linkpic','$type_id','$ishdn','$adduser','$addtime')";
		$result = $mysqli->query($sql);
		if(!$result){
			$handle = $myfun->_mysql_string('<span class="blue">[ 添加飘窗 ]</span> <span class="red">[ 失败 ]</span> '.$arr['linkname']);
			//记录操作日志（失败）
			...
			$handle = $myfun->_mysql_string('<span class="blue">[ 添加飘窗 ]</span> <span class="green">[ 成功 ]</span> '.$arr['linkname']);
			//记录操作日志（成功）
			$insert = "insert into `handle_log_info` (uid,handle,addtime,ip) values ('$uid','$handle','$addtime','$ip')";
			$mysqli->query($insert);
		}
...
<body>
    <div id="admin_cnt">
        <ul class="menu_breadcrumbs">
            <li>当前位置：</li>
            <li>应用模块 >> 广告模块 >>  飘窗添加</li>
            <li id="back"><a href="javascript:history.go(-1);"><span>返回</span></a></li>
        </ul>
        <ul class="main_top_menu">
        	<li class="top_menu"><a href="piaochuanglist.php">飘窗列表</a></li>
        	<li class="top_menu"><a href="piaochuangadd.php" class="active">添加飘窗</a></li>
        </ul>
    	<div id="admin_cnt_inner">
			<div id="addcnt">
            	<form action="" method="post" id="advertform">
            	<table class="table_all">
                  <caption>飘窗添加</caption>
                  <tbody>
                    <tr>
