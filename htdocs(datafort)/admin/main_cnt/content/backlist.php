<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['search'])){
		...
		$myfun->session_set('backkey',$keyword);
	}
	//删除文章
	if($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['act']) && $_GET['act'] == 'del'){  
		...
		if($atcid){
			...
			$sql = "delete from `content_info` where id = '$atcid' and tag = 0";
			$result = $mysqli->query($sql);
			if($result){
				$handle = $myfun->_mysql_string('<span class="blue">[ 文章删除 ]</span> <span class="green">[ 成功 ]</span> [ ID:'.$atcid.' - '.$row['adduser'].' - '.date("Y-m-d",$row['addtime']).' ] '.$row['title_first']).' [ 退稿文章 ]';
				//记录操作日志（成功）
				...
				$handle = $myfun->_mysql_string('<span class="blue">[ 文章删除 ]</span> <span class="red">[ 失败 ]</span> [ ID:'.$atcid.' - '.$row['adduser'].' - '.date("Y-m-d",$row['addtime']).' ] '.$row['title_first']).' [ 退稿文章 ]';
				//记录操作日志（失败）
				$insert = "insert into `handle_log_info` (uid,handle,addtime,ip) values ('$uid','$handle','$addtime','$ip')";
				$mysqli->query($insert);
    ...
    <div id="admin_cnt">
        ...
        <div id="main_top_seach">
            <form action="" method="post">
              <label for="keyword">关键字：</label><input type="text" name="keyword" id="keyword" size="22" style="padding-left:2px;" value="<?php if(!!$myfun->session_get('backkey')){echo $myfun->session_get('backkey');}else{?>请输入关键字……<?php }?>" onfocus="if (value =='请输入关键字……'){value =''}" onblur="if (value ==''){value='请输入关键字……'}" /> &nbsp;
    	...
    	<div id="admin_cnt_inner">
			<div id="listcnt">
            	<table class="table_special">
                    ...
                    <?php
						// 获取检索条件
						$backkey = $myfun->session_get('backkey');
						...
						if($backkey){
							$condition.=" and title_first like '%$backkey%'";
						}
						...
						$page=new Page($total, $num, 'mid='.$mid);
                	...
                	<tr>
                      ...
                      <td>
                        ...
                        <td width="5%" class="jbox"><a href="contentshow.php?mid=<?php echo $row['mid'];?>&atcid=<?php echo $row['id'];?>">预览</a></td>
                        <td width="5%" class="jbox"><a href="contenteditor.php?mid=<?php echo $row['mid'];?>&atcid=<?php echo $row['id'];?>"><span>编辑</span></a></td>
